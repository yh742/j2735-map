...
const CustomMarker = React.memo(({ classes, tracking, highlight, highlightTTL, popup, animateIcons, lat, long, msgType, heading, speed, zoom, mapBearing, ttl }) => {
    let markerSize = msgType === "BSM"? 4: 3;
    let markerHeight = ScaleMarker(lat, zoom, markerSize);
    // (146/278) ratio of image dimension of the car
    // (618/489) ratio of image dimension of the pedestrian
            ...
            { popup? (<div className={classes.popups}>{speed} KPH</div>): null } 
                ...
                style={{
                    filter: highlight? `drop-shadow(0px 0px 15px rgba(255, 255, 255, 1))`: 
                        highlightTTL && ttl < 2? 'invert(24%) sepia(94%) saturate(7169%) hue-rotate(357deg) brightness(98%) opacity(20%) contrast(114%)': 
                        "none",
